...
def get_external_python_sympy(env, bin):
    if env["use_sympy"] is True:
        ...
        sp = subprocess.Popen(
            [bin, "-c", cmd], stdin=None, stderr=None, stdout=subprocess.PIPE
        )
        ...
        if version1 == 0 and version2 < 7:
            env["sympy"] = False
            env["warnings"].append(
                "sympy version too old. Symbolic toolbox and nonlinear PDEs will not be available."
            )
...
def call_python_config(bin=None):
    ...
    if bin is None:
        ...
        # AP: I do not understand the return above.
        return (None, None, ver, sysconfig.get_python_inc())
    ...
    if hasattr(target, "decode"):
        target, libname, ver, pinc = (
            target.decode().strip(),
            libname.decode().strip(),
            ver.decode().strip(),
            pinc.decode().strip(),
        )
    else:
...
def checkPython(env):
    ...
    if env["pythoncmd"] == sys.executable:
        if env["IS_WINDOWS"]:
            ...
            (python_lib_path, python_libs, verstring, python_inc_path) = (
                call_python_config(env["pythoncmd"])
            )
    ...
    # Note: we assume scons is running python 2 in the following.
        ...
        if env["IS_WINDOWS"]:
            ...
            pout = subprocess.Popen(
                [env["pythoncmd"], "-c", cmd], stdout=subprocess.PIPE
            ).stdout.read()
    ...
    if env["sysheaderopt"]:
        ...
        conf.env.Append(CCFLAGS=f"-isystem {python_inc_path}")
        # pass
    else:
...
def checkBoost(env):
    ...
    my_prefix = env["boost_prefix"]
    boost_inc_path = f"{my_prefix}/include"
    boost_lib_path = f"{my_prefix}/lib"
    ...
    if env["sysheaderopt"] == "":
        ...
        env.Append(CCFLAGS=f"-isystem {boost_inc_path}")
    ...
    for line in boosthpp:
        ...
        if ver:
            ...
            if maj <= REQUIRED_BOOST[0] and minor < REQUIRED_BOOST[1]:
                print(
                    "The boost version referenced must be at least version %d.%d "
                    % REQUIRED_BOOST
                    + "(have %d.%d.%d)" % (maj, minor, sub)
                )
                env.Exit(1)
    boosthpp.close()
    ...
    if boostversion >= 106300 and not env["disable_boost_numpy"]:
        ...
        try:
            ...
            boost_numpy_inc_path = f"{my_prefix}/include"
            boost_numpy_lib_path = f"{my_prefix}/lib"
            ...
            print(
                "Warning: Could not find boost/python/numpy.hpp. Building without numpy support."
            )
...
def checkNumpy(env):
    ...
    if conda_prefix:
        ...
        conf.env.Append(
            CPPPATH=[conda_prefix + "/Lib/site-packages/numpy/core/include"]
        )
...
def checkCUDA(env):
    try:
        cuda_inc_path, cuda_lib_path = findLibWithHeader(
            env, "cudart", "thrust/version.h", env["cuda_prefix"], lang="c++"
        )
...
def checkCppUnit(env):
    try:
        cppunit_inc_path, cppunit_lib_path = findLibWithHeader(
            env,
            env["cppunit_libs"],
...
def checkForTrilinos(env):
    ...
    if env["trilinos"]:
        ...
        havelibs = False
    trilinos_inc_path, trilinos_lib_path = findLibWithHeader(
        env,
        env["trilinos_libs"],
        "Amesos2.hpp",
        env["trilinos_prefix"],
    ...
    for check in dependencies:
        print(
            ...
            % (
                check,
                "yes"
                if os.path.isfile(os.path.join(trilinos_inc_path, check))
                else "no",
            )
        )
        if not os.path.isfile(os.path.join(trilinos_inc_path, check)):
            print(
                "Could not find the  Trilinos header file %s (tried looking in directory %s)"
                % (check, trilinos_inc_path)
            )
            env.Exit(1)
    ...
    if os.path.isfile(os.path.join(trilinos_inc_path, "Tpetra_BlockCrsMatrix.hpp")):
        ...
        os.path.join(trilinos_inc_path, "Tpetra_Experimental_BlockCrsMatrix.hpp")
    ):
        print(
            "Checking for %s... %s" % ("Tpetra_Experimental_BlockCrsMatrix.hpp", "yes")
        )
    ...
    if os.path.isfile(
        os.path.join(trilinos_inc_path, "Tpetra_BlockCrsMatrix_Helpers.hpp")
    ):
        print("Checking for %s... %s" % ("Tpetra_BlockCrsMatrix_Helpers.hpp", "yes"))
    elif os.path.isfile(
        os.path.join(
            trilinos_inc_path, "Tpetra_Experimental_BlockCrsMatrix_Helpers.hpp"
        )
    ):
        print(
            "Checking for %s... %s" % ("Tpetra_Experimental_BlockCrsMatrix_Helpers.hpp")
        )
        env.Append(CPPDEFINES=["ESYS_HAVE_TPETRA_EXPERIMENTAL_BLOCKCRSH"])
    else:
        raise RuntimeError(
            "Could not locate the Trilinos Block CRS Matrix Helpers header"
        )

    if os.path.isfile(os.path.join(trilinos_inc_path, "Tpetra_BlockVector.hpp")):
        print("Checking for %s... %s" % ("Tpetra_BlockVector.hpp", "yes"))
    elif os.path.isfile(
        os.path.join(trilinos_inc_path, "Tpetra_Experimental_BlockVector.hpp")
    ):
...
def checkOptionalLibraries(env):
    ...
    if env["netcdf"]:
        if env["netcdf"] == 4:
            ...
            netcdf_inc_path, netcdf_lib_path = findLibWithHeader(
                env, env["netcdf_libs"], "ncVar.h", env["netcdf_prefix"], lang="c++"
            )

        else:
            netcdf_inc_path, netcdf_lib_path = findLibWithHeader(
                env, env["netcdf_libs"], "netcdfcpp.h", env["netcdf_prefix"], lang="c++"
            )
    ...
    mkl_lib_path = ""
    if env["mkl"]:
        mkl_inc_path, mkl_lib_path = findLibWithHeader(
            env, env["mkl_libs"], "mkl_pardiso.h", env["mkl_prefix"], lang="c++"
        )
    ...
    umfpack_lib_path = ""
    if env["umfpack"]:
        umfpack_inc_path, umfpack_lib_path = findLibWithHeader(
            env, env["umfpack_libs"], "umfpack.h", env["umfpack_prefix"], lang="c++"
        )
    ...
    if env["mumps"]:
        if env["usempi"]:
            mumps_inc_path, mumps_lib_path = findLibWithHeader(
                env, env["mumps_libs"], "mumps_mpi.h", env["mumps_prefix"], lang="c++"
            )
        else:
            mumps_inc_path, mumps_lib_path = findLibWithHeader(
                env,
    ...
    if env["lapack"] != 0:
        ...
        if env["longindices"]:
            ...
            if env["lapack"] == 1:
                print(
                    "LAPACK requires index type = int. Set longindices to False or disable LAPACK."
                )
                env.Exit(1)
        else:
            if env["mkl"]:
            ...
            try:
                lapack_inc_path, lapack_lib_path = findLibWithHeader(
                    env, libs, header, prefix, lang="c++"
                )
    ...
    silo_lib_path = ""
    if env["silo"]:
        silo_inc_path, silo_lib_path = findLibWithHeader(
            env, env["silo_libs"], "silo.h", env["silo_prefix"], lang="c++"
        )
    ...
    visit_lib_path = ""
    if env["visit"]:
        visit_inc_path, visit_lib_path = findLibWithHeader(
            env,
            env["visit_libs"],
    ...
    mpi_lib_path = ""
    if env["usempi"]:
        mpi_inc_path, mpi_lib_path = findLibWithHeader(
            env, env["mpi_libs"], "mpi.h", env["mpi_prefix"], lang="c++"
        )
    ...
    parmetis_lib_path = ""
    if env["parmetis"]:
        parmetis_inc_path, parmetis_lib_path = findLibWithHeader(
            env, env["parmetis_libs"], "parmetis.h", env["parmetis_prefix"], lang="c++"
        )
        ...
        if major is not None:
            env["parmetis_version"] = "%d.%d.%d" % (
                major,
                minor,
                0 if sub is None else sub,
            )
            if env["longindices"]:
                # ParMETIS version 3.x does not support 64-bit indices
                if major < 4:
                    print(
                        "Sorry, cannot use ParMETIS version < 4.0 with 64-bit index types. Set longindices to False or disable ParMETIS."
                    )
                    env.Exit(1)
                else:
                    # check if ParMETIS was built with 64-bit indices
                    conf = Configure(env.Clone())
                    idxsize = conf.CheckTypeSize(
                        "idx_t", "#include <parmetis.h>", "C++"
                    )
                    if idxsize != 8:
                        print(
                            "Sorry, ParMETIS was not compiled with 64-bit indices. Set longindices to False or disable/rebuild ParMETIS."
                        )
                        env.Exit(1)
        else:
    ...
    if env["zlib"]:
        try:
            zlib_inc_path, zlib_lib_path = findLibWithHeader(
                env, env["zlib_libs"], "zlib.h", env["zlib_prefix"], lang="c"
            )
    ...
    if env["compressed_files"]:
        try:
            boost_inc_path, boost_lib_path = findLibWithHeader(
                env,
                env["compression_libs"],
