...
from geom import TetGeom, precompute_geometry_from_knt_ijk
...
def _build_jax_amg_hierarchy_from_geom(
    geom: TetGeom, amg: str = "sa", gauge: float = 0.0
) -> JaxAmgHierarchy:
    ...
    try:
        ...
        raise RuntimeError(
            "pyamg is required for building AMG hierarchy (pip install pyamg)"
        ) from exc
    ...
    for lvl in ml.levels:
        ...
        Dinv_list.append(
            jax.device_put(1.0 / (A_csr.diagonal().astype(np.float64) + 1e-30))
        )
...
def get_or_build_jax_amg_hierarchy(
    geom: TetGeom, amg: str = "sa", gauge: float = 0.0
) -> JaxAmgHierarchy:
...
def energy_and_grad_A(
    m_nodes: jnp.ndarray, A_nodes: jnp.ndarray, geom: TetGeom, Ms_lookup: jnp.ndarray
    ...
    return brown_energy_and_grad_from_m(
        m_nodes=m_nodes, A_nodes=A_nodes, geom=geom, Ms_lookup=Ms_lookup
    )
...
def _solve_A_jax_cg_compMG_core(
    ...
    R_t,
    Dinv_t,
    L_c,  # tuples (hier parts) + coarse Cholesky factor
    ...
    *,
    ...
    def _make_per_component_vcycle_M(
        A_list,
        P_list,
        R_list,
        Dinv_list,
    ...
    M = _make_per_component_vcycle_M(
        A_t, P_t, R_t, Dinv_t, nu_pre, nu_post, omega, coarse_iters, coarse_omega
    )
...
def _solve_U_jax_cg_compMG_core(
    ...
    R_t,
    Dinv_t,
    L_c,  # tuples (AMG levels) + optional coarse Cholesky factor
    ...
    *,
    ...
    # ---- Run CG ----
    x_sol, info = jsp_linalg.cg(
        A_apply, b, x0=x0, tol=tol, atol=0.0, maxiter=maxiter, M=M
    )
...
def read_mesh_params_from_p2(p2_path: str) -> tuple[float, float | None, float | None]:
    """Read [mesh] 'size' (default 1.0e-9 if missing) and optional K/KL from a .p2 file.
    Returns (size, K_opt, KL_opt).
    """
    DEFAULT_SIZE = 1.0e-9
    size: float = DEFAULT_SIZE
    K_opt: float | None = None
    KL_opt: float | None = None
...
def prepare_shells_and_geom(
    *,
    ...
    volume_scalefactor = jnp.where(
        body_vol_sum > 0.0, 1.0 / body_vol_sum, jnp.asarray(1.0, dtype=jnp.float64)
    )
