...
def generate_distribution_all_three(knt, conn, mat_id, tol, max_attempts, seed=None):
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        if (
            (abs(avg_x - 0.5) < tol)
            and (abs(avg_y - 0.5) < tol)
            and (abs(avg_z - 0.5) < tol)
        ):
            print(
                f"[ok] Accepted after {attempt} attempt(s): "
                f"<mx>_V={avg_x:.4f}, <my>_V={avg_y:.4f}, <mz>_V={avg_z:.4f}"
            )
            return easy_axes, (avg_x, avg_y, avg_z), attempt

    raise RuntimeError(
        f"Failed to satisfy all three averages within tol={tol} after {max_attempts} attempts."
    )
...
def write_krn(path, easy_axes, K1, Js, A, summary=None):
    ...
    with open(path, "w", encoding="utf-8") as f:
        if summary is not None:
            ...
            f.write(
                f"# Volume-averaged <mx>={avg_x:.6f} <my>={avg_y:.6f} <mz>={avg_z:.6f}\n"
            )
...
def main():
    ...
    ap.add_argument(
        "--mesh", required=True, help="Path to mesh .npz (knt, ijk) produced by mesh.py"
    )
    ap.add_argument(
        "--K1", type=float, default=0.7e6, help="Anisotropy constant K1 [J/m^3]"
    )
    ap.add_argument(
        "--Js", type=float, default=0.8, help="Saturation polarization Js [T]"
    )
    ap.add_argument(
        "--A", type=float, default=7.6e-11, help="Exchange constant A [J/m]"
    )
    ap.add_argument(
        "--tol", type=float, default=0.001, help="Tolerance for averages vs 0.5"
    )
    ap.add_argument("--out", type=str, default="combined.krn", help="Output .krn path")
    ap.add_argument(
        "--max-attempts", type=int, default=20000, help="Maximum random trials"
    )
    ...
    if gmin != 1:
        print(
            f"[warn] mat_id minimum is {gmin}, expected 1. Proceeding but check your mesh."
        )
    print(
        f"[info] Mesh loaded: grains={gmax} (IDs {gmin}..{gmax}), elements={conn.shape[0]:,d}"
    )
